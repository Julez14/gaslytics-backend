            ...
            req.on("end", async () => {
                try {
                    ...
                    const progressCallback = (stage, progress, message) => {
                        ...
                        console.log(`📊 Progress: ${progress}% - ${stage}${message ? ` (${message})` : ""}`);
                    ...
                    // Process the video
                    const result = await processVideoWithTwelveLabs(videoUrl, progressCallback);
                    if (result.success) {
                        sendLog("success", `Processing completed! Found ${result.analysisResult?.clips?.length || 0} clips`);
                    }
                    else {
...
server.listen(PORT, () => {
    console.log(`🚀 TwelveLabs Processing Server running on http://localhost:${PORT}`);
    console.log(`📊 Health check: http://localhost:${PORT}/api/health`);
    console.log(`🎬 Video processing: POST http://localhost:${PORT}/api/process-video`);
