...
export const validateAnalysisData = (analysisData) => {
    try {
        ...
        const validatedClips = analysisData.clips.map((clip, index) => {
            ...
            if (!clip.startTime ||
                ...
                !clip.confidence ||
                !clip.solution) {
                throw new Error(`Clip ${index + 1} is missing required fields. Required: startTime, endTime, transcript, tactic, justification, confidence, solution`);
            }
            // Validate time format (HH:MM:SS.ss or HH:MM:SS)
            const timeRegex = /^\d{2}:\d{2}:\d{2}(\.\d{2})?$/;
            if (!timeRegex.test(clip.startTime) || !timeRegex.test(clip.endTime)) {
                throw new Error(`Clip ${index + 1} has invalid time format. Expected: HH:MM:SS.ss or HH:MM:SS`);
            }
            // Validate tactic is one of the allowed values
            if (!validTactics.includes(clip.tactic)) {
                throw new Error(`Clip ${index + 1} has invalid tactic. Allowed: ${validTactics.join(", ")}`);
            }
            // Validate confidence is a number between 0-100
            if (typeof clip.confidence !== "number" ||
                clip.confidence < 0 ||
                clip.confidence > 100) {
                throw new Error(`Clip ${index + 1} has invalid confidence. Must be a number between 0-100`);
            }
            ...
            if (typeof clip.transcript !== "string" ||
                ...
                throw new Error(`Clip ${index + 1} has invalid field types. transcript, justification, and solution must be strings`);
            }
            return {
...
export async function processVideoWithTwelveLabs(videoUrl, progressCallback) {
    try {
        console.log("🚀 Starting TwelveLabs video processing...");
        progressCallback?.("Initializing TwelveLabs client", 5, "Setting up API connection...");
        ...
        progressCallback?.("Uploading video to TwelveLabs", 15, "Sending video URL to TwelveLabs for processing...");
        ...
        progressCallback?.("Video uploaded", 25, `Task created with ID: ${task.id}`);
        // Monitor the video indexing process
        console.log("\n🔄 Monitoring indexing progress...");
        progressCallback?.("Processing video", 30, "TwelveLabs is indexing the video...");
        let lastStatus = "";
        await task.waitForDone(50, (task) => {
            if (task.status !== lastStatus) {
                ...
                if (task.status === "indexing") {
                    progressCallback?.("Indexing video", 50, "TwelveLabs is analyzing video content...");
                }
        ...
        progressCallback?.("Video ready", 65, `Video successfully indexed with ID: ${task.videoId}`);
        ...
        try {
            ...
            progressCallback?.("Prompt loaded", 70, "Analysis prompt loaded successfully");
        }
        catch (error) {
        ...
        progressCallback?.("Analyzing content", 70, "TwelveLabs is running the analysis prompt...");
        ...
        const { isValid, error: validationError, validatedData, } = validateAnalysisData(parsedAnalysis);
        if (!isValid) {
        ...
        progressCallback?.("Generating insights", 98, "Using Gemini to generate summary...");
